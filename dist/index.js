 ...
 */\n`;
        ...
        spinner.succeed(c.green(`TypeScript enums generated at ${ENUMS_FILE} (${enumDefinitions.length} enums)`));
        return true;
    }
    catch (error) {
...
async function processSchemaFile(schemaFilePath) {
    const spinner = ora(c.cyan('Processing schema file to improve readability...')).start();
    try {
...
async function processSchema(schema) {
    ...
    if (!typesOnly) {
        const pullSpinner = ora(c.cyan(`Fetching ${schemaName} schema from database...`)).start();
        try {
    ...
    try {
        ...
        if (typesOnly) {
            try {
                ...
                schemaAccessSpinner.succeed(c.green(`Found schema file at ${FINAL_SCHEMA_FILE}`));
            }
            catch {
                ...
                schemaAccessSpinner.succeed(c.green(`Found schema file at ${SCHEMA_FILE}`));
            }
        }
        else {
        ...
        schemaAccessSpinner.fail(c.red(`Schema file not found in either location`));
    ...
    try {
        ...
        if (snapshotFile) {
            metaSpinner.succeed(c.green(`Found metadata snapshot at ${join(META_DIR, snapshotFile)}`));
        }
        else {
            metaSpinner.info(c.blue('No metadata snapshot found, will use schema.ts directly'));
        }
    }
    catch {
        metaSpinner.info(c.blue('No metadata directory found, will use schema.ts directly'));
    }
    // Process the generated schema to create types
    const typesSpinner = ora(c.cyan('Generating TypeScript types from schema...')).start();
    ...
    typesSpinner.text = c.cyan(`Found ${c.bold(tableMatches.length.toString())} tables and ${c.bold(enumMatches.length.toString())} enums in the schema`);
...
`;
    ...
    typesSpinner.succeed(c.green(`TypeScript types generated at ${TYPES_FILE}`));
...
`;
    ...
    if (!typesOnly) {
        const moveSpinner = ora(c.cyan('Moving schema file and cleaning up...')).start();
        try {
            // Copy schema.ts to the final location
            await fs.copyFile(SCHEMA_FILE, FINAL_SCHEMA_FILE);
            moveSpinner.text = c.cyan('Schema file copied, removing migrations directory...');
            ...
            moveSpinner.succeed(c.green('Schema file moved and migrations directory removed'));
        }
        catch (error) {
            moveSpinner.fail(c.red('Failed to move schema file or clean up migrations directory'));
            console.error(c.red(String(error)));
        }
    }
    // Run eslint --fix on the generated files
    const lintSpinner = ora(c.cyan('Running ESLint to fix any style issues...')).start();
    try {
        ...
        lintSpinner.warn(c.yellow('ESLint encountered issues but processing will continue'));
        console.warn(c.yellow.dim('⚠️ ESLint errors:'), c.dim(error instanceof Error ? error.message : String(error)));
    }
    console.log();
    console.log(c.green.bold(`✅ ${schemaName} schema processing completed successfully`));
    console.log(c.dim(`${tableMatches.length} tables and ${enumMatches.length} enums processed`));

...
async function main() {
  ...
  if (!databaseURL && !typesOnly) {
    console.error(
      c.red(
        'DATABASE_URL environment variable is required to pull the schema from the database.'
      )
    );
    process.exit(1);
  }
  ...
  try {
    console.log(
      c.bold.cyan(`Processing schema: ${schemaName}`)
    );
    console.log(
      c.dim(`Mode: ${typesOnly ? 'Types only' : 'Full pull + type generation'}`)
    );
    console.log();
